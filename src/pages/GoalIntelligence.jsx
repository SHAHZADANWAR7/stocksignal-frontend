...
export default function GoalIntelligence() {
  ...
  const loadData = async () => {
    ...
    const [goalsData, holdingsData] = await Promise.all([
      awsApi.getPortfolioGoal({ email: JSON.parse(localStorage.getItem('stocksignal_user_attributes'))?.email }),
      awsApi.getStockBatch({ symbols: [], forceRefresh: true })
    ]);
    
    // Also load paper trading portfolio if regular holdings are empty
    let allHoldings = holdingsData || [];
    if (allHoldings.length === 0) {
      const paperPortfolio = await awsApi.getStockBatch({ symbols: [], forceRefresh: true });
